...
function additionalInfo(){
  const labels={
    proformaInvoice:"Proforma Invoice",sourceBuyer:"Source Buyer",sourceOrder:"Source Order",
...
function fieldHTML(f){
  ...
  if(type==="select"){
    const extra=name==="category" ? [] : allTextValues(name);
...
function backupData(){
  try{
    ...
    const payload={backupType:'SAG FASHON LTD ERP FULL DATA BACKUP',backupMode:'FULL',backupVersion:'V17',createdAt:new Date().toISOString(),data:snapshot};
...
function setPageState(page,sub=""){
  try{localStorage.setItem(PAGE_STATE_KEY,JSON.stringify({page,sub:sub||""}));}catch(e){}
}
function getPageState(){
  try{
    const x=JSON.parse(localStorage.getItem(PAGE_STATE_KEY)||"");
    if(x&&titles[x.page]) return x;
  }catch(e){}
...
function go(p,sub=""){
  ...
  else if(["raw","dyed","grey","reqDyeing","reqKnitting"].includes(p))entry(p);
  else if(p==="stock"){
    if(sub&&["raw","dyed","grey"].includes(sub)) stock(sub);
    else stockMenu();
...
function renderTable(t){
  ...
  tbl.innerHTML='<div class="tableWrap dataScroll"><table id="entryTable"><thead><tr>'+fs.map(f=>'<th>'+titleCaseText(f[1])+'</th>').join("")+'<th>Actions</th></tr></thead><tbody>'+D[t].map(r=>'<tr>'+fs.map(f=>'<td>'+esc(f[0]==="quantity"?formatQty(r[f[0]]):r[f[0]])+'</td>').join("")+'<td><button class="edit" onclick="editRow(\''+t+'\',\''+r.id+'\')">Edit</button><button class="del" onclick="delRow(\''+t+'\',\''+r.id+'\')">Delete</button></td></tr>').join('')+'</tbody><tfoot><tr class="subtotalRow">'+fs.map((f,i)=>'<td data-subtotal-col="'+i+'">'+(i===0?'Subtotal':(i===qi?'0.00':''))+'</td>').join('')+'<td></td></tr></tfoot></table></div>';
  document.getElementById('entryTable').dataset.subtotalCols=String(qi);
...
function groups(t){
  ...
  D[t].forEach(r=>{
    ...
    const tr=String(r.transaction||'').trim().toUpperCase();
    ...
    if(STOCK_RECEIVE[t].has(tr)){
      // Only the original stock-receiving transaction supplies Received Date.
      // Returns never become the Received Date.
      if(tr===primaryReceive){
        const d=String(r.date??'').trim();
        if(d && (!m[k].receivedDate || d < m[k].receivedDate))m[k].receivedDate=d;
        m[k].received+=N(r.quantity);
      }else if(tr.includes('RETURN FROM')){
        m[k].returnQty+=N(r.quantity);
      }else{
        m[k].received+=N(r.quantity);
      }
...
function stockMenu(){
  content.innerHTML='<div class="choices"><button class="choice" onclick="go(\'stock\',\'raw\')">Grey Yarn Stock</button><button class="choice" onclick="go(\'stock\',\'dyed\')">Dyed Yarn Stock</button><button class="choice" onclick="go(\'stock\',\'grey\')">Grey Fabrics Stock</button></div><div class="panel" id="sv">Select Stock.</div>';
}
function stock(t){
...
function statement(t){
  ...
  if(t==="knitting"){
    ...
    const addK=(x,amount,kind,factory)=>{
      const buyer=x.buyer||"Not Specified",order=x.order||"Not Specified",fac=factoryValue(x,factory);
      const k=[buyer,order,fac].join("|");
      if(!mm[k])mm[k]={buyer,order,factory:fac,del:0,recv:0};
      if(kind==="del")mm[k].del+=amount;
      else if(kind==="return")mm[k].del-=amount;
      else if(kind==="recvDelivery")mm[k].recv-=amount;
      else mm[k].recv+=amount;
    };
    ...
    D.grey.forEach(x=>{const tr=String(x.transaction||"").toUpperCase();if(tr==="GREY FABRICS RECEIVED FROM KNITTING")addK(x,N(x.quantity),"recv",x.knittingFactory);if(tr==="GREY FABRICS DELIVERY TO KNITTING")addK(x,N(x.quantity),"recvDelivery",x.knittingFactory)});
    const rows=Object.values(mm).sort((a,b)=>(a.buyer+"|"+a.order+"|"+a.factory).localeCompare(b.buyer+"|"+b.order+"|"+b.factory)).map(v=>[v.buyer,v.order,v.factory,v.del,v.recv,v.del-v.recv]);
...
function renderStatement(title,h,rows){
  ...
  document.getElementById('statementTable').dataset.subtotalCols=qtyCols.join(",");
  updateTableSubtotals('statementTable',qtyCols);
...
function updateTableSubtotals(tableId,qtyCols){
  ...
  const totals={};qtyCols.forEach(i=>totals[i]=0);
  table.querySelectorAll('tbody tr').forEach(tr=>{
    if(tr.style.display==='none')return;
    qtyCols.forEach(i=>{const n=parseFloat(String(tr.cells[i]?.textContent||'').replace(/,/g,''));if(Number.isFinite(n))totals[i]+=n;});
  });
...
function filterTable(tableId,query,mode="exact"){
  ...
  table.querySelectorAll('tbody tr').forEach(tr=>{
    ...
    const isMatch=cells.some((cell,i)=>{
      if(tableId==="entryTable"&&i===cells.length-1)return false;
      const value=String(cell.textContent??"").trim().toLowerCase();
      if(mode==="contains")return value.includes(q);
      return exactSearchMatch(value,q);
    });
    tr.style.display=isMatch?"":"none";
  });
  const configured=table.dataset.subtotalCols||"";
  const qtyCols=configured.split(",").map(x=>Number(x)).filter(Number.isInteger);
  updateTableSubtotals(tableId,qtyCols);
}
...
function sagFullExcelBackup(){
  try{
    save();
    if(typeof XLSX==='undefined'){alert('Excel library is unavailable. Please reload the ERP with internet access.');return}
    ...
    Object.keys(snapshot).forEach(k=>{
      ...
      if(k==="additional" && v && typeof v==="object" && !Array.isArray(v)){
        // Additional Info: each field is a separate Excel column and values run vertically.
        const fields=ADDITIONAL_FIELDS.filter(field=>Array.isArray(v[field]));
        const maxRows=fields.reduce((n,field)=>Math.max(n,v[field].length),0);
        rows=Array.from({length:maxRows},(_,i)=>{
          const row={};
          fields.forEach(field=>{row[field]=String(v[field][i]??"")});
          return row;
        });
      }else if(Array.isArray(v)){
        rows=v;
      }else if(v && typeof v==="object"){
        rows=Object.keys(v).map(field=>({Field:field,Value:Array.isArray(v[field])?JSON.stringify(v[field]):String(v[field]??'')}));
      }

      const ws=rows.length?XLSX.utils.json_to_sheet(rows):XLSX.utils.aoa_to_sheet([['No Data']]);
      let s=String(k).replace(/[\\/\?\*\[\]\:]/g,'_').slice(0,31)||'Data',n=1,o=s;
      while(wb.SheetNames.includes(s)){const q='_'+n++;s=o.slice(0,31-q.length)+q}
      XLSX.utils.book_append_sheet(wb,ws,s);
    });

    XLSX.writeFile(wb,'SAG_FASHON_LTD_ERP_FULL_BACKUP_'+sagBackupStamp()+'.xlsx');
    alert('Full Excel Backup Completed Successfully.\n\nAdditional Info has been exported with each field as a separate column.');
  }catch(e){alert('Excel Backup Failed!\n\n'+e.message)}
}
function sagFullExcelRestore(input){const f=input.files[0];if(!f)return;if(typeof XLSX==='undefined'){alert('Excel library is unavailable. Please reload the ERP with internet access.');input.value='';return}const r=new FileReader();r.onload=()=>{try{const wb=XLSX.read(r.result,{type:'array'}),x={};wb.SheetNames.forEach(s=>{
  const rows=XLSX.utils.sheet_to_json(wb.Sheets[s],{defval:''});
  if(s==='additional'){
    const a={};
    // New backup format: each Additional Info field is a separate column,
    // and its saved values are listed vertically under that column.
    const hasColumnFormat=rows.length&&Object.keys(rows[0]).some(k=>ADDITIONAL_FIELDS.includes(k));
    if(hasColumnFormat){
      ADDITIONAL_FIELDS.forEach(field=>{
        a[field]=rows.map(row=>String(row[field]??'').trim()).filter(Boolean);
      });
    }else{
      // Keep compatibility with older Field/Value Excel backups.
      rows.forEach(row=>{
        if(row.Field){
          a[row.Field]=String(row.Value??'').trim()?(()=>{
            try{return JSON.parse(row.Value)}
            catch(_){return String(row.Value)}
          })():[];
        }
      });
    }
