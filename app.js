...
function stock(t){
  ...
  document.getElementById('stockSearch').addEventListener('input',e=>filterTable('stockTable',e.target.value));
}
...
function renderStatement(title,h,rows){
  ...
  document.getElementById("statementSearch").addEventListener('input',e=>filterTable('statementTable',e.target.value));
}
...
function filterTable(tableId,query){
  ...
  const qtyCols=[...table.querySelectorAll('tfoot [data-subtotal-col]')].map(c=>Number(c.dataset.subtotalCol)).filter(Number.isInteger);
